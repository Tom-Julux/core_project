...
class SAM2CameraPredictor(SAM2Base):
    ...
    def perpare_data(
        ...
        if isinstance(img, np.ndarray):
            img_np = img
            #img_np = cv2.resize(img_np, (image_size, image_size), cv2.INTER_NEAREST) / 255.0
            img_np = np.array(Image.fromarray(img_np).resize((image_size, image_size)))/ 255.0
    ...
    def add_new_points(
        ...
        if prev_out is not None and prev_out["pred_masks"] is not None:
            prev_sam_mask_logits = prev_out["pred_masks"].to(self.condition_state["device"], non_blocking=True)
            # Clamp the scale of prev_sam_mask_logits to avoid rare numerical issues.
            prev_sam_mask_logits = torch.clamp(prev_sam_mask_logits, -32.0, 32.0)
        current_out, _ = self._run_single_frame_inference(
            output_dict=obj_output_dict,  # run on the slice of a single object
    ...
    def add_new_mask(
        ...
        if mask_H != self.image_size or mask_W != self.image_size:
            ...
            mask_inputs = (mask_inputs >= self.mask_threshold).float()
        else:
