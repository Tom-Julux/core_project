...
class InteractiveSegmentationWidgetSAM2(QWidget):
    ...
    def build_gui(self):
        ...
        self.layerselect_a = setup_layerselect(
            _layout, self._viewer, Image, function=lambda: self.run_predict_in_thread()
        )
        ...
        self.multi_mask_ckbx = setup_checkbox(_layout, "Use Multi-Mask", True, function=lambda: self.update_hyperparameters())
        ...
        _group_box, _layout = setup_vcollapsiblegroupbox(_scroll_layout, text="Manual Control:", collapsed=True)
    ...
    def run_predict_in_thread(self):
        ...
        def _worker():
            self.predict()
        _worker().start()  # Start the worker thread
