...
class InteractiveSegmentationWidgetSAM2_2D_T(QWidget):
    ...
    def build_gui(self):
        ...
        self.swapp_button = setup_iconbutton(
            _layout,
            "Swap (views)",
            "erase",
            self._viewer.theme,
    ...
    def run_propagate_in_thread(self,*args,**kwargs):
        @thread_worker
        def _worker():
    ...
    def propagate(self):
        ...
        reverse = get_value(self.reverse_ckbx)
        
        img_layer = get_value(self.layerselect_a)[1]

        self.predictor.mask_threshold = get_value(self.threshold_slider)

        next_frame_idx = self._viewer.dims.current_step[self._viewer.dims.order[0]]+ (1 if not reverse else -1)
