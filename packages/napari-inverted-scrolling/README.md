...
The behavior can be inverted using the `Invert Scrolling` and `Reset Scrolling` commands in the napari command palette (`Ctrl+P`). The `Scroll through slices` and `Scroll to zoom` commands are added as alternative names.